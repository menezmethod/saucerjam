...
class Game {
  ...
  updateAuth(session, error = null) {
    ...
    const initials = email ? email.split("@")[0].replace(/[^a-z0-9]/gi, "").slice(0, 2).toUpperCase() || "SJ" : "SJ";
    ...
    $("account-status").textContent = user ? "Cloud linked" : "Guest pilot";
    $("account-status-dot").dataset.online = user ? "true" : "false";
    $("account-sigil").textContent = initials;
    $("account-heading").textContent = user ? "Pilot control" : "Save your flight";
    $("account-copy").textContent = user ? "Your identity and online records are linked to this account." : "Sign in to carry your callsign and records between devices.";
    $("account-email").textContent = email || "Device pilot";
    $("account-sync").textContent = user ? "Records synced" : "Browser only";
    $("account-context").textContent = user ? "Cloud-linked pilot. Records follow this account." : "Guest flights stay on this browser. Sign in above to carry your pilot between devices.";
